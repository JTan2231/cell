PIPELINE_SCHEMA=1
PRODUCT_ID=cast
PRODUCT_NAME=Cast
PRODUCT_DIR=cast
CI_RESOURCE_CLASS=heavy
RELEASE_BRANCH=main
DEPLOY_PROFILE=custom
DEPLOY_CONFLICT_KEYS='global'
CARGO_MANIFEST=Cargo.toml
CARGO_PACKAGES=cast
CARGO_OFFLINE=0
CLIPPY_KEEP_GOING=1
TEST_NO_FAIL_FAST=1
CI_SHELL_CHECKS='sh|cast/release.sh
sh|cast/packaging/macos/deploy-user.sh
sh|cast/packaging/macos/test-deploy-user.sh
zsh|cast/packaging/macos/cast'
CI_RUN_CHECKS='darwin|cast/packaging/macos/test-deploy-user.sh'
CI_PROVIDER_VALIDATION_PHASE=before-rust
RELEASE_UNITS='cast|Cast|package|cast/Cargo.toml|cast-|1'
RELEASE_METADATA_NO_DEPS=1
RELEASE_BINARY_CHECKS='cast|target/release/cast|cast'
PROVIDERS='cast|cast|cast/chancery|3'
