...
use crate::api::{Attachment, Message, Receipt, ReceivedMessage, ReceivedPage, ReceivedPageRequest, ReplyOptions};
...
impl Client {
    ...
    pub async fn send_with_options(
        ...
        let payload = serde_json::to_vec(&Payload { body: &message.body, attachments: encoded })
        ...
        let id = output.trim_end_matches(['\r', '\n']).strip_prefix("Accepted ")
        ...
        if id.is_empty() || id.len() > 256 || !id.bytes().all(|byte| byte.is_ascii_graphic()) {
            return Err(AppError::new("Email returned an invalid acceptance receipt"));
        }
    ...
    pub async fn list_received(&self, page: &ReceivedPageRequest) -> AppResult<ReceivedPage> {
        ...
        let mut arguments = vec!["receive".to_owned(), "list".to_owned(), "--limit".to_owned(), page.limit.to_string()];
        if let Some(id) = &page.after {
            arguments.extend(["--after".to_owned(), id.clone()]);
        }
        let output = self.invoke(&arguments, None, MAX_COMMAND_OUTPUT_BYTES).await?;
        ...
        {
            return Err(AppError::new("Email returned an invalid received email page"));
        }
        Ok(result)
    }
    ...
    pub async fn get_received(&self, id: &str) -> AppResult<ReceivedMessage> {
        ...
        if message.id != id {
            return Err(AppError::new("Email returned a different received email ID"));
        }
        Ok(message)
    }

    async fn invoke(&self, arguments: &[String], input: Option<Vec<u8>>, limit: usize) -> AppResult<Vec<u8>> {
        if !self.executable.is_absolute() {
            return Err(AppError::new("Email executable must be an absolute installed wrapper path"));
        }
        ...
        let operation = async {
            ...
            if !status.success() {
                return Err(AppError::new("installed Email command failed; acceptance or read is not confirmed"));
            }
            Ok(output)
        };
        tokio::time::timeout(Duration::from_secs(120), operation).await
            .map_err(|_| AppError::new("installed Email command timed out; send acceptance may be unknown"))?
    }
}
