...
pub(crate) fn validate_id(id: &str) -> AppResult<()> {
    ...
    {
        return Err(AppError::new("received email ID must contain 1 to 256 ASCII letters, digits, - or _"));
    }
    Ok(())
}
...
pub(crate) fn validate_page(page: &ReceivedPageRequest) -> AppResult<()> {
    if !(1..=100).contains(&page.limit) {
        return Err(AppError::new("received email page limit must be between 1 and 100"));
    }
...
pub(crate) async fn list_received(page: &ReceivedPageRequest) -> AppResult<ReceivedPage> {
    ...
    {
        return Err(AppError::new("Resend returned an invalid received email page"));
    }
...
pub(crate) async fn get_received(id: &str) -> AppResult<ReceivedMessage> {
    ...
    .await?;
    if result.id != id {
        return Err(AppError::new("Resend returned a different received email ID"));
    }
    Ok(result)
}
    ...
    for attempt in 0..=RETRY_DELAYS.len() {
        ...
        let mut response = match response {
            ...
            Err(_) => {
                ...
                return Err(AppError::new("unable to read received email through Resend"));
            }
        };
        ...
        if !status.is_success() {
            return Err(AppError::new(format!("Resend rejected receiving read with HTTP {status}")));
        }
        ...
        if interrupted {
            ...
            return Err(AppError::new("unable to read complete received email response"));
        }
        return serde_json::from_slice(&bytes)
            .map_err(|_| AppError::new("Resend returned invalid received email data"));
    }
    Err(AppError::new("email exhausted its Resend receiving attempts"))
}
