...
manifest_path=email/crates/email/Cargo.toml
provider_path=email/chancery/provider.json
ci_path=email/ci.sh
...
reported_version=$(target/release/email --version) \
    || fail 'unable to read the release binary version'
[ "$reported_version" = "email $new_version" ] \
...
if ! git tag -a "$tag" -m "Email v$new_version"; then
    printf '%s\n' \
        "release.sh: tagging failed; the local release commit was preserved" \
        "release.sh: retry with: git tag -a $tag -m 'Email v$new_version'" >&2
    exit 1
fi
