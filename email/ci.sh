...
if [ "${EMAIL_CI_TIMEOUT_ACTIVE:-0}" != 1 ]; then
    export EMAIL_CI_TIMEOUT_ACTIVE=1
    timeout_command=
...
/bin/zsh -n packaging/macos/email
...
[ -n "$package_version" ] && [ "$provider_release" = "$package_version" ] || {
    printf 'ci.sh: Email provider release %s does not match package version %s\n' \
...
printf '%s\n' '==> rustfmt'
cargo fmt --manifest-path "$WORKSPACE_MANIFEST" --package email -- --check

printf '%s\n' '==> clippy'
cargo clippy --manifest-path "$WORKSPACE_MANIFEST" \
    --package email --all-targets --locked --keep-going -- \
    -D warnings \
    -F unsafe_code \
    -D clippy::all \
    -D clippy::pedantic \
