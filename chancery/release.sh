...
manifest_path=chancery/crates/chancery/Cargo.toml
provider_manifest_path=chancery/provider/provider.json
ci_path=chancery/ci.sh
...
[ -f "$provider_manifest_path" ] \
    || fail "provider manifest not found: $provider_manifest_path"
...
[ "$provider_version" = "$current_version" ] \
    || fail "provider release $provider_version does not match package version $current_version"
...
provider_manifest_tmp="$provider_manifest_path.release.$$"
rollback_version_files=false
cleanup() {
    status=$?
    trap - 0 1 2 15
    set +e
    rm -f "$manifest_tmp" "$provider_manifest_tmp"
    if [ "$rollback_version_files" = true ]; then
        git restore --staged --worktree -- \
            "$manifest_path" "$provider_manifest_path" "$lockfile_path"
        printf 'release.sh: restored %s, %s, and %s after failure\n' \
            "$manifest_path" "$provider_manifest_path" "$lockfile_path" >&2
    fi
    exit "$status"
}
trap cleanup 0
...
awk -v old="$current_version" -v new="$new_version" '
    ...
    { print }
    END { if (changed != 1) exit 1 }
' "$provider_manifest_path" >"$provider_manifest_tmp" \
    || fail "unable to update the provider release in $provider_manifest_path"
mv "$provider_manifest_tmp" "$provider_manifest_path"
...
reported_version=$(target/release/chancery --version) \
    || fail 'unable to read the release binary version'
[ "$reported_version" = "chancery $new_version" ] \
...
expected_files=$(printf '%s\n' \
    "$lockfile_path" "$manifest_path" "$provider_manifest_path")
[ "$changed_files" = "$expected_files" ] \
    || fail "unexpected files changed during release checks"
git diff --check

git add -- "$manifest_path" "$provider_manifest_path" "$lockfile_path"
...
if ! git tag -a "$tag" -m "Chancery v$new_version"; then
    printf '%s\n' \
        "release.sh: tagging failed; the local release commit was preserved" \
        "release.sh: retry with: git tag -a $tag -m 'Chancery v$new_version'" >&2
    exit 1
fi
