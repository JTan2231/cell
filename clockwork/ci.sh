...
printf '%s\n' '==> shell and packaging'
for script in release.sh packaging/macos/deploy-user.sh \
    packaging/macos/uninstall-user.sh packaging/macos/test-deploy-user.sh; do
    [ -f "$script" ] \
        || { printf 'ci.sh: missing %s\n' "$script" >&2; exit 1; }
    sh -n "$script"
done
...
' "$SCRIPT_DIR/crates/clockwork/Cargo.toml")
...
printf '%s\n' '==> rustfmt'
cargo fmt --manifest-path "$WORKSPACE_MANIFEST" --package clockwork -- --check

printf '%s\n' '==> clippy'
cargo clippy --manifest-path "$WORKSPACE_MANIFEST" \
    --package clockwork --all-targets --locked --keep-going -- \
    -D warnings \
    -F unsafe_code \
    -D clippy::all \
    -D clippy::pedantic \
