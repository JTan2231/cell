...
manifest_path=clockwork/crates/clockwork/Cargo.toml
provider_path=clockwork/chancery/provider.json
ci_path=clockwork/ci.sh
...
provider_version=$(awk -F '"' \
    '/"release"[[:space:]]*:/ { print $4; exit }' "$provider_path")
[ "$provider_version" = "$current_version" ] \
    || fail "provider $provider_version does not match package $current_version"
...
reported_version=$(target/release/clockwork --version) \
    || fail 'unable to read release binary version'
[ "$reported_version" = "clockwork $new_version" ] \
...
if ! git tag -a "$tag" -m "Clockwork v$new_version"; then
