...
cp "$SCRIPT_DIR/decisions-observer" "$release/bin/decisions-observer"
cat >"$release/libexec/decisions" <<'EOF'
#!/bin/sh
env | sort >"$HOME/capture"
printf '%s\n' "$@" >>"$HOME/capture"
EOF
chmod 0755 "$release/bin/decisions-observer" "$release/libexec/decisions"
HOME="$home" SECRET_MUST_NOT_LEAK=value RESEND_API_KEY=must-not-leak \
    /bin/sh "$release/bin/decisions-observer"
...
rm -f "$capture"
HOME="$home" /bin/sh "$release/bin/decisions-observer" \
...
if HOME="$home" /bin/sh "$release/bin/decisions-observer" \
    >/dev/null 2>"$temporary/private-maintenance.stderr"; then
    printf '%s\n' 'observer runner accepted a non-private maintenance gate' >&2
    exit 1
fi
...
if HOME="$home" /bin/sh "$release/bin/decisions-observer" \
    ...
    printf '%s\n' 'observer runner accepted a hard-linked maintenance gate' >&2
    exit 1
fi
...
if HOME="$home" /bin/sh "$release/bin/decisions-observer" \
    ...
    printf '%s\n' 'observer runner accepted an invalid maintenance gate' >&2
    exit 1
fi
