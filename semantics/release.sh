...
manifest_path=semantics/Cargo.toml
provider_path=semantics/chancery/provider.json
ci_path=semantics/ci.sh
...
reported_version=$(target/release/semantics --version) \
    || fail 'unable to read release binary version'
[ "$reported_version" = "semantics $new_version" ] \
...
if ! git tag -a "$tag" -m "Semantics v$new_version"; then
