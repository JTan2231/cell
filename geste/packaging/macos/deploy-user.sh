...
if [ -d "$SCRIPT_DIR/../share/chancery/geste" ]; then
    SOURCE_CHANCERY="$SCRIPT_DIR/../share/chancery/geste"
else
...
Install or update the current user's macOS Geste CLI and product-owned
Chancery provider. Deployment does not initialize or migrate episode storage.
...
operator_uid=$(id -u)
[ "$operator_uid" -ne 0 ] || fail 'run as the Geste operator, not root'
...
validate_bundle() {
    ...
    expected_bundle_tree=$(printf '%s\n' \
        ...
        ./entries/episode-capture.json \
        ./entries/episode-explore.json \
        ./entries/install-operate.json \
        ./manuals \
        ./manuals/develop-change.md \
        ./manuals/episode-capture.md \
        ./manuals/episode-explore.md \
        ./manuals/install-operate.md \
        ./provider.json \
        | LC_ALL=C sort)
    [ "$actual_bundle_tree" = "$expected_bundle_tree" ] \
        || fail "Chancery bundle tree is not the exact Geste v0.1 layout: $bundle"
    bundle_provider=$(awk -F '"' \
        '/"id"[[:space:]]*:/ { print $4; exit }' \
        "$bundle/provider.json")
    [ "$bundle_provider" = geste ] \
...
validate_release_selector() {
    ...
    printf '%s\n' "$selector" | grep -Eq '^releases/[0-9a-f]{64}$' \
        || fail "invalid Geste release selector: $selector"
}
...
validate_installed_release() {
    ...
    for directory in "$inspected_path" "$inspected_path/bin" \
        ...
        "$inspected_path/share/chancery/geste"; do
    ...
    expected_release_tree=$(printf '%s\n' \
        ...
        ./share \
        ./share/chancery \
        ./share/chancery/geste \
        ./share/chancery/geste/entries \
        ./share/chancery/geste/entries/develop-change.json \
        ./share/chancery/geste/entries/episode-capture.json \
        ./share/chancery/geste/entries/episode-explore.json \
        ./share/chancery/geste/entries/install-operate.json \
        ./share/chancery/geste/manuals \
        ./share/chancery/geste/manuals/develop-change.md \
        ./share/chancery/geste/manuals/episode-capture.md \
        ./share/chancery/geste/manuals/episode-explore.md \
        ./share/chancery/geste/manuals/install-operate.md \
        ./share/chancery/geste/provider.json \
        | LC_ALL=C sort)
    [ "$actual_release_tree" = "$expected_release_tree" ] \
        || fail "installed release tree is not the exact Geste v0.1 layout: $selector"
    [ -f "$inspected_path/bin/geste" ] \
        && [ ! -L "$inspected_path/bin/geste" ] \
        && [ -x "$inspected_path/bin/geste" ] \
    ...
    validate_bundle "$inspected_path/share/chancery/geste"
    ...
    [ "$(sed -n '2p' "$inspected_manifest")" = 'product=geste' ] \
...
candidate_version=$("$binary_path" --version) \
    || fail 'unable to read the Geste candidate version'
case "$candidate_version" in
    'geste '*) version=${candidate_version#geste } ;;
...
STATE_DIR="$install_home/Library/Application Support/Geste"
...
CLI_PATH="$CLI_DIR/geste"
CHANCERY_STATE="$install_home/Library/Application Support/Chancery"
CHANCERY_PROVIDERS="$CHANCERY_STATE/providers"
CHANCERY_LINK="$CHANCERY_PROVIDERS/geste"
EXPECTED_CLI="$INSTALL_DIR/current/bin/geste"
EXPECTED_CHANCERY="$INSTALL_DIR/current/share/chancery/geste"
...
mkdir "$UPDATE_LOCK" 2>/dev/null \
    || fail "another Geste deployment is active: $UPDATE_LOCK"
lock_created=1
...
if [ -n "$old_current" ]; then
    ...
    fail 'installed selectors have no current Geste release'
fi
...
if [ -d "$release_path" ]; then
    ...
    install -m 0755 "$binary_path" "$temporary_release/bin/geste"
    install -m 0755 "$SOURCE_DEPLOYER" \
        "$temporary_release/package/deploy-user.sh"
    cp -R "$SOURCE_CHANCERY" \
        "$temporary_release/share/chancery/geste"
    {
        printf '%s\n' 'format=1' 'product=geste'
        printf 'release_id=%s\n' "$release_id"
        printf 'version=%s\n' "$version"
...
installed_version=$(HOME="$install_home" "$CLI_PATH" --version) \
    || fail 'installed Geste version check failed'
[ "$installed_version" = "geste $version" ] \
    || fail "installed Geste reported an unexpected version: $installed_version"
HOME="$install_home" "$CLI_PATH" --help >/dev/null \
    || fail 'installed Geste help check failed'
[ -f "$CHANCERY_LINK/provider.json" ] \
    || fail 'installed Geste Chancery provider is unavailable'
validate_installed_release "$(readlink "$CURRENT_LINK")"

committed=1
printf 'Installed Geste release %s\n' "$release_id"
printf 'Command: %s\n' "$CLI_PATH"
printf 'Chancery provider: %s\n' "$CHANCERY_LINK"
printf '%s\n' 'Database: not initialized by deployment; run `geste init` separately'
