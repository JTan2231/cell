    ...
    match operation {
        Operation::Inspect => {
            ...
            Ok(reply(
                ...
                json!({"current":selection(&snapshot),"installed":snapshot,"runtime":status,"harness_executable":harness(&health)?,"maintenance_products":["annals","krisis","semantics","crm","todo","weaver"],"after":[]}),
            ))
        }
        Operation::Hold => {
