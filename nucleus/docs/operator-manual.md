...
New Platter state defaults to `~/.local/share/platter`. If only the predecessor
`~/.local/share/job-packets` directory exists, it remains the default. If both
exist, supply global `--state-dir`. The rename moves no artifacts and rewrites
no templates, packet IDs, stage requests, or send receipts. While the runner is
inactive, back up the database, configuration, template, packets, and frozen
editions together.
