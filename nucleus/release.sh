...
provider_path=nucleus/chancery/provider.json
...
[ -f "$provider_path" ] || fail 'Nucleus Chancery provider manifest not found'
...
cleanup() {
    ...
    if [ "$rollback_version_files" = true ]; then
        git restore --staged --worktree -- \
            "$manifest_path" "$lock_path" "$provider_path"
        printf 'release.sh: restored %s, %s, and %s after failure\n' \
            "$manifest_path" "$lock_path" "$provider_path" >&2
    fi
    exit "$status"
}
trap cleanup 0
...
expected_files=$(printf '%s\n' "$lock_path" "$manifest_path" "$provider_path")
[ "$changed_files" = "$expected_files" ] \
    || fail "files other than $manifest_path, $lock_path, and $provider_path changed during release checks"
git diff --check

git add -- "$manifest_path" "$lock_path" "$provider_path"
