...
`relevance.jsonl` records the query classes and expected IDs for a small
deterministic library. The relevance test constructs the complete seed,
...
The relevance gate requires exact topic and distinctive phrase cases to appear
in the first three results, `Recall@10` of at least 0.90, and grouping that does
not reduce scoped-query recall. The test additionally reports `Recall@5`,
...
Typo matching is outside the current search boundary. The misspelling
`Transactons` is therefore recorded as an expected miss and
