...
manifest_path=pratica/crates/pratica/Cargo.toml
provider_path=pratica/chancery/provider.json
ci_path=pratica/ci.sh
...
reported_version=$(target/release/pratica --version) \
    || fail 'unable to read release binary version'
[ "$reported_version" = "pratica $new_version" ] \
...
if ! git tag -a "$tag" -m "Pratica v$new_version"; then
