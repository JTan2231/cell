...
for script in release.sh packaging/macos/deploy-user.sh \
    packaging/macos/test-deploy-user.sh; do
    [ -f "$script" ] \
...
printf '%s\n' '==> rustfmt'
cargo fmt --manifest-path "$WORKSPACE_MANIFEST" --package pratica -- --check

printf '%s\n' '==> clippy'
cargo clippy --manifest-path "$WORKSPACE_MANIFEST" \
    --package pratica --all-targets --locked --keep-going -- \
    -D warnings \
    -F unsafe_code \
    -D clippy::all \
    -D clippy::pedantic \
