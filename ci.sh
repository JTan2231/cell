...
usage() {
    printf '%s\n' \
        'Usage: ./ci.sh [nucleus|annals|todo|chancery|weaver|email|conversations|krisis|decisions|semantics|geste|pratica|clockwork|crm|vizier]...'
}

if [ "$#" -eq 0 ]; then
    set -- nucleus annals todo chancery weaver email conversations krisis \
        semantics geste pratica clockwork crm vizier
fi
...
if [ "$nucleus_selected$annals_selected$todo_selected$chancery_selected$weaver_selected$email_selected$conversations_selected$krisis_selected$semantics_selected$geste_selected$pratica_selected$clockwork_selected$crm_selected$vizier_selected" = \
    11111111111111 ]
then
