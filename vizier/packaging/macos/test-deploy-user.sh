...
python3 "$SCRIPT_DIR/../../../deployment/generate.py" --check --product vizier
temporary=$(mktemp -d "${TMPDIR:-/tmp}/vizier-deploy-test.XXXXXX")
...
candidate_three="$temporary/vizier-three"
candidate_mismatch="$temporary/vizier-mismatch"
...
' "$SCRIPT_DIR/../../crates/vizier/Cargo.toml")
...
cp -R "$SCRIPT_DIR/../../chancery" "$share/vizier"
...
state="$home/Library/Application Support/Vizier"
install_dir="$state/install"
releases="$install_dir/releases"
cli="$home/.local/bin/vizier"
providers="$home/Library/Application Support/Chancery/providers"
provider="$providers/vizier"
[ -L "$cli" ]
[ "$(readlink "$cli")" = "$install_dir/current/bin/vizier" ]
[ -L "$install_dir/current" ]
[ ! -e "$install_dir/previous" ]
[ -L "$provider" ]
[ "$(readlink "$provider")" = \
    "$install_dir/current/share/chancery/vizier" ]
[ -x "$install_dir/current/bin/vizier" ]
...
[ ! -e "$state/vizier.db" ]
[ ! -e "$state/vizier.db-wal" ]
[ ! -e "$state/vizier.db-shm" ]
[ "$(stat -f '%Lp' "$state")" = 700 ]
...
[ "$first_provider_path" = "$first_release_path/share/chancery/vizier" ]
...
printf '%s\n' '# tampered binary' >>"$install_dir/current/bin/vizier"
...
install -m 0755 "$candidate_one" "$install_dir/current/bin/vizier"

current_provider_manifest="$install_dir/current/share/chancery/vizier/provider.json"
printf '%s\n' 'tampered provider' >>"$current_provider_manifest"
...
install -m 0644 "$share/vizier/provider.json" "$current_provider_manifest"
...
[ "$second_provider_path" = "$second_release_path/share/chancery/vizier" ]
[ ! -e "$state/vizier.db" ]
...
[ "$rolled_back_provider_path" = \
    "$second_release_path/share/chancery/vizier" ]
...
printf '%s\n' 'foreign command' >"$foreign_command_home/.local/bin/vizier"
chmod 0755 "$foreign_command_home/.local/bin/vizier"
...
grep -Fx 'foreign command' "$foreign_command_home/.local/bin/vizier" >/dev/null
