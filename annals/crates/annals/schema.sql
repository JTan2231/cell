...
-- The library role is physical database identity, not configuration. It is
-- selected once during initialization and cannot be changed in place.
...
-- One immutable row per producer acceptance. These rows are operational
-- library state and the read model for the decision-account feed; raw account
-- Markdown remains in its inbox envelope and eventual retained work.
