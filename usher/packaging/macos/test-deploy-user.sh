...
python3 "$SCRIPT_DIR/../../../deployment/generate.py" --check --product usher
temporary=$(mktemp -d "${TMPDIR:-/tmp}/usher-deploy-test.XXXXXX")
usher_test_home="$temporary/Operator Home"
package="$temporary/package/macos"
share="$temporary/package/share/chancery"
candidate_template="$temporary/usher.template"
candidate_one="$temporary/usher-one"
candidate_two="$temporary/usher-two"
candidate_three="$temporary/usher-three"
candidate_mismatch="$temporary/usher-mismatch"
...
' "$SCRIPT_DIR/../../crates/usher/Cargo.toml")
...
mkdir -p "$usher_test_home" "$package" "$share"
install -m 0755 "$SCRIPT_DIR/deploy-user.sh" "$package/deploy-user.sh"
cp -R "$SCRIPT_DIR/../../chancery" "$share/usher"
...
case "${1:-}" in
    --version) printf '%s\n' 'usher @VERSION@'; exit 0 ;;
    --help) printf '%s\n' 'fake Usher help'; exit 0 ;;
    marker) printf '%s\n' '@MARKER@'; exit 0 ;;
esac
exit 64
EOF
...
deploy() {
    candidate=$1
    shift
    HOME="$usher_test_home" "$package/deploy-user.sh" \
        --binary "$candidate" --home "$usher_test_home" "$@"
}
...
state="$usher_test_home/Library/Application Support/Usher"
install_dir="$state/install"
releases="$install_dir/releases"
cli="$usher_test_home/.local/bin/usher"
providers="$usher_test_home/Library/Application Support/Chancery/providers"
provider="$providers/usher"
[ -L "$cli" ]
[ "$(readlink "$cli")" = "$install_dir/current/bin/usher" ]
[ -L "$install_dir/current" ]
[ ! -e "$install_dir/previous" ]
[ -L "$provider" ]
[ "$(readlink "$provider")" = \
    "$install_dir/current/share/chancery/usher" ]
[ -x "$install_dir/current/bin/usher" ]
[ -x "$install_dir/current/package/deploy-user.sh" ]
[ -f "$install_dir/current/manifest.txt" ]
[ -f "$provider/provider.json" ]
[ "$(HOME="$usher_test_home" "$cli" marker)" = one ]
[ ! -e "$state/usher.db" ]
[ ! -e "$state/usher.db-wal" ]
[ ! -e "$state/usher.db-shm" ]
[ "$(stat -f '%Lp' "$state")" = 700 ]
...
[ "$first_provider_path" = "$first_release_path/share/chancery/usher" ]
...
printf '%s\n' '# tampered binary' >>"$install_dir/current/bin/usher"
...
install -m 0755 "$candidate_one" "$install_dir/current/bin/usher"

current_manual="$install_dir/current/share/chancery/usher/manuals/recognition-inspect.md"
...
install -m 0644 "$share/usher/manuals/recognition-inspect.md" "$current_manual"
...
: >"$usher_test_home/fail-installed"
...
[ "$rolled_back_provider_path" = \
    "$second_release_path/share/chancery/usher" ]
rm "$usher_test_home/fail-installed"
[ "$(HOME="$usher_test_home" "$cli" marker)" = two ]
[ ! -e "$state/usher.db" ]
...
printf '%s\n' 'foreign command' >"$foreign_command_home/.local/bin/usher"
chmod 0755 "$foreign_command_home/.local/bin/usher"
...
grep -Fx 'foreign command' "$foreign_command_home/.local/bin/usher" >/dev/null
