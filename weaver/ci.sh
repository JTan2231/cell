...
if [ "${WEAVER_CI_TIMEOUT_ACTIVE:-0}" != 1 ]; then
    export WEAVER_CI_TIMEOUT_ACTIVE=1
...
[ -n "$package_version" ] && [ "$provider_release" = "$package_version" ] || {
    printf 'ci.sh: Weaver provider release %s does not match package version %s\n' \
...
cargo clippy --manifest-path "$WORKSPACE_MANIFEST" \
    --package weaver --all-targets --locked --keep-going -- \
    -D warnings \
    -F unsafe_code \
    -D clippy::all \
    -D clippy::pedantic \
