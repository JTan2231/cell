...
Initialization resolves the corpus from Bazaar, creates a shuffle seed, and leaves
...
`mentor.installation.operate` for supported release, binding, and
...
Prompt preparation requires initialized private Bazaar state and a complete cell.prompts.mentor selection. The default database is ~/.local/share/bazaar/bazaar.sqlite3; callers accept an absolute CELL_BAZAAR_DATABASE override. Reads fail without creating state or using embedded fallback text.

Read `cell.prompts.mentor` with Bazaar's supported `get` interface. Its content
...
cell.prompts.mentor --file /absolute/selection.json`. Use an explicit
