...
impl Grader {
    ...
    ///
    /// # Errors
    /// Returns an error when the current user's Nucleus client cannot be configured.
    ...
    ///
    /// # Errors
    /// Rejects empty grading input, an invalid request, or failed JSON encoding.
    pub fn prepare_request(
        ...
        serde_json::to_string(&request).map_err(|_| fail("Cannot encode the grading request"))
    }
    ...
    ///
    /// # Errors
    /// Rejects invalid or mismatched requests and unavailable admission readiness.
    /// Returns errors for failed or timed-out provider reads and unresolved admission.
    pub async fn advance(&self, request_json: &str) -> Result<Progress> {
            ...
            .map_err(|_| fail("Nucleus admission is unresolved; retry the same request"))?
        {
            ...
            Err(_) => {
                return Err(fail(
                    "Nucleus admission is unresolved; retry the same request",
                ));
            }
        };
    ...
    ///
    /// # Errors
    /// Rejects invalid or mismatched requests. Returns provider errors, timeouts,
...
fn build_request(job_id: &str, input: &Input, cwd: &Path) -> Result<JobRequestV1> {
    ...
    let prompt =
        serde_json::to_string(input).map_err(|_| fail("Cannot encode the grading input"))?;
...
fn decode_request(request_json: &str) -> Result<JobRequestV1> {
    ...
    if request != expected {
        return Err(fail(
            "The persisted request does not match Mentor's grading policy",
        ));
    }
    Ok(request)
}
...
fn verify_job(request: &JobRequestV1, job: &JobV1) -> Result<()> {
    ...
    {
        return Err(fail(
            "The Nucleus attempt does not match the grading request",
        ));
    }
    Ok(())
}
...
fn verify_cancellation_owner(id: &JobId, job: &JobV1) -> Result<()> {
    ...
    {
        return Err(fail(
            "The Nucleus job does not belong to this Mentor grading run",
        ));
    }
    Ok(())
}
...
fn progress(request: &JobRequestV1, job: &JobV1) -> Result<Progress> {
    ...
    match job.summary.state {
        ...
        JobState::Completed => {
            ...
            {
                return Ok(Progress::Failed(
                    "The grading completion is inconsistent".to_owned(),
                ));
            }
            let Some(output) = &attempt.output else {
                return Ok(Progress::Failed(
                    "The grading attempt has no final critique".to_owned(),
                ));
            };
            ...
            {
                return Ok(Progress::Failed(
                    "The final critique is missing or too large".to_owned(),
                ));
            }
