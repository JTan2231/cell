...
pub fn automated(message: &ReceivedMessage) -> bool {
    header(message, "auto-submitted").is_some_and(|v| !v.trim().eq_ignore_ascii_case("no"))
        || header(message, "precedence").is_some_and(|v| {
            matches!(
...
/// Build a bounded reference chain ending with the incoming message ID.
///
/// # Errors
/// Rejects an incoming message without a usable RFC Message-ID.
pub fn references(message: &ReceivedMessage) -> Result<Vec<String>> {
    if !valid_message_id(&message.message_id) {
        return Err(fail("incoming message has no usable RFC Message-ID"));
    }
    ...
    if let Some(value) = header(message, "references") {
        ...
        for id in value
            ...
            .rev()
        {
            if id != message.message_id && !result.iter().any(|old| old == id) {
                result.push(id.to_owned());
            }
        }
    }
    result.push(message.message_id.clone());
    while result.iter().map(|id| id.len() + 1).sum::<usize>() > 8192 {
        result.remove(0);
    }
    Ok(result)
}

/// Read the provider's receipt timestamp as Unix seconds UTC.
///
/// # Errors
/// Rejects a provider timestamp that is not valid RFC 3339.
pub fn received_at(message: &ReceivedMessage) -> Result<i64> {
    chrono::DateTime::parse_from_rfc3339(&message.created_at)
        .map(|time| time.timestamp())
        .map_err(|_| fail("received email has an invalid provider timestamp"))
}
...
///
/// # Errors
/// Rejects non-inline attachments, exceeded text or HTML limits, NUL bytes,
/// ambiguous quoted replies, and messages with no answer text.
pub fn extract_answer(message: &ReceivedMessage) -> Result<String> {
    if message
        .attachments
        .iter()
        .any(|a| a.content_disposition.as_deref() != Some("inline"))
    {
        return Err(fail(
            "Please paste your complete answer into the email body. Mentor does not read attached answers.",
        ));
    }
    ...
    for line in normalized.lines() {
        ...
        if !in_code {
            ...
            if quoted && !trimmed.is_empty() {
                return Err(fail(
                    "Please send your complete answer above the quoted email. Mentor cannot separate this inline reply reliably.",
                ));
            }
        }
        answer.push(line);
    ...
    if text.is_empty() {
        return Err(fail(
            "I could not find an answer in this reply. Please paste your complete answer above the quoted email.",
        ));
    }
    Ok(text)
}
...
fn html_text(source: &str) -> Result<String> {
    if source.len() > 1024 * 1024 {
        return Err(fail(
            "Please resend your answer as plain text under 128 KiB.",
        ));
    }
    ...
    for node in document.tree.root().descendants() {
        let excluded = node.ancestors().any(|parent| match parent.value() {
            Node::Element(element) => {
                matches!(element.name(), "script" | "style" | "head") || quote_element(element)
            }
            _ => false,
        });
        if excluded {
        ...
        match node.value() {
            Node::Element(element) => {
                if quote_element(element) {
                    saw_quote = true;
                    continue;
                }
                if matches!(
                    element.name(),
                    "p" | "div" | "br" | "li" | "tr" | "pre" | "h1" | "h2" | "h3"
                ) {
                    output.push('\n');
                }
            }
            Node::Text(text) => {
                if saw_quote && !text.trim().is_empty() {
                    return Err(fail(
                        "Please send your complete answer above the quoted email. Mentor cannot separate this inline HTML reply reliably.",
                    ));
                }
