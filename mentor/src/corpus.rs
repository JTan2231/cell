...
impl Corpus {
    ...
    fn validate(&self) -> crate::Result<()> {
        ...
        for problem in &self.problems {
            ...
            let titles: Vec<_> = problem.markdown.lines().filter_map(level_one_title).collect();
...
fn validate_rubric(rubric: &Rubric) -> crate::Result<()> {
    ...
    let titles: Vec<_> = rubric.markdown.lines().filter_map(level_one_title).collect();
    ...
    for (index, dimension) in dimensions.iter().enumerate() {
        ...
        if name.is_empty() || !names.insert(name) {
            return Err(invalid("rubric dimension names must be nonempty and unique"));
        }
    }
    Ok(())
}
