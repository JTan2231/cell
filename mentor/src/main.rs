...
#[derive(Parser)]
#[command(name="mentor",version,about="Daily system design problems and email critiques")]
struct Cli {
...
enum Command {
    ...
    Configure {
        ...
        #[arg(long)] receiving_domain: Option<String>,
        #[arg(long)] email_executable: Option<PathBuf>,
    },
    Pause,
    Resume,
    ...
    Doctor,
    Schedule { #[arg(value_enum)] operation: ScheduleOperation },
    Maintenance { #[command(subcommand)] operation: MaintenanceOperation },
    /// Back up drained metadata and initialize or accept the supported schema.
    Migrate { #[arg(long)] backup: PathBuf },
}

#[derive(Clone, ValueEnum)]
enum ScheduleOperation { Enable, Disable, Status }
...
async fn main() -> ExitCode {
    ...
    match run(cli.command).await {
        Ok(data) => {
            ...
            if cli.json { println!("{reply}"); }
            else { println!("{}",serde_json::to_string_pretty(&reply).unwrap_or_else(|_|reply.to_string())); }
            ExitCode::SUCCESS
        }
        Err(error) => {
            ...
            if cli.json { println!("{reply}"); } else { eprintln!("{reply}"); }
            ExitCode::FAILURE
        }
    }
}
...
async fn run(command: Command) -> Result<Value> {
    ...
    match command {
        ...
        Command::Schedule { operation } => mentor::installation::schedule(&root,match operation {
            ScheduleOperation::Enable=>"enable",ScheduleOperation::Disable=>"disable",ScheduleOperation::Status=>"status",
        }),
        ...
        Command::Configure { time,timezone,receiving_domain,email_executable } => {
            let _admission = mentor::gate(&root).enter()?;
            let _lock = runner_lock(&root)?;
            let store = Store::open(&root)?;
            let mut config = store.config()?;
            if let Some(time) = time {
                let (hour,minute) = time.split_once(':').ok_or_else(||fail("time must be HH:MM"))?;
                if hour.len()!=2 || minute.len()!=2 { return Err(fail("time must be HH:MM")); }
                config.hour=hour.parse().map_err(|_|fail("time must be HH:MM"))?;
                config.minute=minute.parse().map_err(|_|fail("time must be HH:MM"))?;
            }
            if let Some(timezone) = timezone { config.timezone=timezone; }
            if let Some(domain) = receiving_domain { config.receiving_domain=domain; }
            if let Some(executable) = email_executable { config.email_executable=executable; }
        ...
        Command::ImportCorpus { path } => {
            ...
            File::open(path)?.take(8*1024*1024+1).read_to_string(&mut body)?;
...
fn set_pause(root:&Path,paused:bool)->Result<Value> {
    ...
    let _admission=if paused { gate.recover()? } else { gate.enter()? };
    let _lock=runner_lock(root)?;
    let store=Store::open(root)?;
    let mut config=store.config()?;
...
fn maintenance_status(root:&Path)->Result<Value> {
    ...
    let (outstanding,runner_idle)=match runner_lock(root) {
        Ok(_lock)=> {
            let count=if root.join("mentor.sqlite3").try_exists()? { Store::open(root)?.outstanding_work()? } else { 0 };
            (Some(count),true)
        }
        Err(_)=>(None,false),
    };
    Ok(json!({"maintenance":{"protocol_version":1,"holds":gate.holds,
...
async fn maintenance(root:&Path,operation:MaintenanceOperation)->Result<Value> {
    let gate=mentor::gate(root);
    match operation {
        MaintenanceOperation::Hold { owner }=> { gate.hold(&owner)?; }
        MaintenanceOperation::Status=>{}
        MaintenanceOperation::Drain=> {
            if gate.status()?.holds.is_empty() { return Err(fail("maintenance drain requires a hold")); }
        ...
        MaintenanceOperation::Release { owner }=> {
            ...
            if status["maintenance"]["drained"]!=true {
                return Err(fail("maintenance cannot be released until admitted work has drained"));
            }
            gate.release(&owner)?;
        }
    }
...
fn doctor(root:&Path)->Result<Value> {
    ...
    let (config,state)=if root.join("mentor.sqlite3").try_exists()? {
        let store=Store::open(root)?;
        (store.config()?,json!({"initialized":true,"schema_version":1,"status":store.status()?}))
    } else { (Config::defaults()?,json!({"initialized":false,"schema_version":null})) };
    let email_present=fs::metadata(&config.email_executable).is_ok_and(|metadata|metadata.is_file() && metadata.mode()&0o111!=0);
    Ok(json!({"local_state_compatible":true,"state":state,"installation":installation,
...
fn migrate(root:&Path,backup:&Path)->Result<Value> {
    if !backup.is_absolute() || backup==root.join("mentor.sqlite3") { return Err(fail("backup must be a separate absolute path")); }
    private_directory(root)?;
    let gate=mentor::gate(root);
    let owner=std::env::var("CELL_DEPLOYMENT_RUN_ID").ok();
    let _admission=if let Some(owner)=&owner { gate.enter_for(owner)? } else { gate.enter()? };
    let _lock=runner_lock(root)?;
    ...
    if existed {
        ...
        if store.outstanding_work()?!=0 { return Err(fail("migration requires drained work; pending content must not enter a backup")); }
        ...
        if content_count!=0 { return Err(fail("migration found retained temporary content; backup refused")); }
        drop(store);
        private_directory(backup.parent().ok_or_else(||fail("backup has no parent"))?)?;
        copy_backup(&source,backup)?;
    }
    let store=Store::initialize(root)?;
    Ok(json!({"schema_version":1,"backup":if existed {Some(backup)} else {None},"status":store.status()?}))
}

fn copy_backup(source:&Path,destination:&Path)->Result<()> {
    match OpenOptions::new().write(true).create_new(true).mode(0o600).open(destination) {
        Ok(mut backup)=> {
            std::io::copy(&mut File::open(source)?,&mut backup)?;
            backup.flush()?;
            backup.sync_all()?;
            File::open(destination.parent().ok_or_else(||fail("backup has no parent"))?)?.sync_all()?;
            Ok(())
        }
        Err(error) if error.kind()==std::io::ErrorKind::AlreadyExists=> {
            let metadata=fs::symlink_metadata(destination)?;
            if !metadata.is_file() || metadata.file_type().is_symlink() || metadata.nlink()!=1 || metadata.mode()&0o077!=0
                || fs::read(source)?!=fs::read(destination)? {
                return Err(fail("existing migration backup does not match drained state"));
            }
            Ok(())
        }
