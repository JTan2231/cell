...
use crate::store::{Assignment, Config, Incoming, Store, WORK_LIFETIME, SEND_WINDOW};
...
pub async fn tick(root: &Path, recovery: bool) -> Result<Value> {
    let _admission = if recovery { crate::gate(root).recover()? } else { crate::gate(root).enter()? };
    ...
    if let Ok(grader) = &grader {
        if !matches!(timeout(Duration::from_secs(15),cancel_expired(root,&store,grader)).await,Ok(Ok(()))) {
    ...
    if !config.paused && !recovery {
        if select_daily(&store,&config,crate::now()).is_err() { errors.push("daily_selection_failed"); }
        if !matches!(timeout(Duration::from_secs(20),poll(&store,&email,crate::now())).await,Ok(Ok(()))) { errors.push("reply_poll_failed"); }
    }
    if let Ok(grader) = &grader {
        if !matches!(timeout(Duration::from_secs(20),advance_grading(root,&store,grader)).await,Ok(Ok(()))) { errors.push("grading_pending_or_failed"); }
    }
    if !matches!(timeout(Duration::from_secs(20),send_pending(&store,&email)).await,Ok(Ok(()))) { errors.push("email_submission_unresolved"); }
    store.expire(crate::now())?;
    store.set_meta("last_tick_completed_at",&crate::now().to_string())?;
    errors.sort_unstable(); errors.dedup();
    store.set_meta("last_tick_errors",&errors.join(","))?;
    Ok(json!({"paused":config.paused,"recovery":recovery,"completed_at":crate::now(),"error_codes":errors}))
}
...
fn select_daily(store: &Store, config: &Config, now: i64) -> Result<()> {
    let Some(date) = config.due_date(now)? else { return Ok(()); };
    if store.assigned_today(&date)? { return Ok(()); }
    let (corpus_id,corpus) = store.active_corpus()?;
    let used = store.used_problem_ids()?;
    let seed = store.meta("shuffle_seed")?.ok_or_else(||fail("problem selection seed is absent"))?;
    let problem = corpus.problems.iter().filter(|problem|!used.contains(&problem.id)).min_by_key(|problem| {
        let mut hash = Sha256::new(); hash.update(seed.as_bytes()); hash.update([0]); hash.update(problem.id.as_bytes());
        hex::encode(hash.finalize())
    });
    let Some(problem) = problem else { return Ok(()); };
    let token = crate::random_token()?;
    let assignment = Assignment {
        date,problem_id:problem.id.clone(),corpus_id,reply_to:format!("mentor.{token}@{}",config.receiving_domain),token,
    };
    let payload = serde_json::to_string(&mail::problem_email(&assignment,&problem.title,&problem.markdown))?;
...
async fn poll(store: &Store, email: &Client, now: i64) -> Result<()> {
    ...
    for _ in 0..MAX_PAGES_PER_TICK {
        let page = match email.list_received(&ReceivedPageRequest { limit:100,after:after.clone() }).await {
            Ok(page) => page,
            Err(_) => {
        ...
        for reference in &page.data {
            if store.seen(&reference.id)? { continue; }
            if let Some(assignment) = match_assignment(store,reference)? {
                ...
                let full = email.get_received(&reference.id).await.map_err(|_|fail("Email receiving retrieval failed"))?;
                if full.id != reference.id { return Err(fail("Email returned a different received record")); }
                if match_assignment(store,&full)?.as_ref().map(|a|&a.date)!=Some(&assignment.date) {
        ...
        let next = page.data.last().ok_or_else(||fail("Email returned an empty unfinished page"))?.id.clone();
...
fn match_assignment(store: &Store, message: &ReceivedMessage) -> Result<Option<Assignment>> {
    ...
    for recipient in message.to.iter().chain(message.received_for.iter()) {
        let Some(token) = mail::assignment_token(recipient) else { continue; };
        let Some(assignment) = store.assignment(&token)? else { continue; };
        if mail::mailbox(recipient).as_deref()!=Some(assignment.reply_to.as_str()) { continue; }
        if matched.as_ref().is_some_and(|old|old.date!=assignment.date) {
...
fn capture_reply(store: &Store, assignment: &Assignment, message: &ReceivedMessage, now: i64) -> Result<()> {
    let timestamp = mail::received_at(message).ok().filter(|value|*value<=now+300);
    let received = timestamp.unwrap_or(now);
    let authorized_sender = mail::mailbox(&message.from).as_deref()==Some(email::api::recipient());
    ...
    let extracted = if ignored || expired || !usable_parent { None } else { Some(mail::extract_answer(message)) };
    let state = if ignored { "ignored" } else if expired { "expired" } else if !usable_parent { "failed" } else { "pending" };
    let incoming = Incoming {
        id:message.id.clone(),assignment_date:assignment.date.clone(),state:state.into(),received_at:received,
        expires_at:received+WORK_LIFETIME,message_id:message.message_id.clone(),references,
        answer:extracted.as_ref().and_then(|result|result.as_ref().ok()).cloned(),request_json:None,job_id:None,
    };
    let response = if let Some(Err(extraction)) = extracted {
        let corpus = store.corpus(&assignment.corpus_id)?;
        let problem = corpus.problem(&assignment.problem_id).ok_or_else(||fail("assigned problem is absent"))?;
        let reply = mail::response_email(assignment,&incoming,&problem.title,&extraction.to_string());
...
async fn advance_grading(root: &Path, store: &Store, grader: &Grader) -> Result<()> {
    ...
    let selected = pending.iter().find(|item|item.state=="grading").or_else(||pending.first());
    let Some(incoming) = selected else { return Ok(()); };
    if incoming.state!="grading" && !store.cancellation_jobs()?.is_empty() {
        return Err(fail("previous grading cancellation is still pending"));
    }
    if incoming.expires_at<=crate::now() { return Ok(()); }
    let assignment = store.assignment_date(&incoming.assignment_date)?;
    let corpus = store.corpus(&assignment.corpus_id)?;
    let problem = corpus.problem(&assignment.problem_id).ok_or_else(||fail("assigned problem is absent"))?;
    let job_id = incoming.job_id.clone().unwrap_or_else(||uuid::Uuid::now_v7().to_string());
    let scratch = root.join("scratch").join(&job_id);
    crate::store::private_directory(&scratch)?;
    let request = match &incoming.request_json {
        ...
        None => {
            ...
            let request = grader.prepare_request(&job_id,&problem.markdown,&corpus.rubric.markdown,answer,&scratch)?;
    ...
    match grader.advance(&request).await? {
        ...
        Progress::Complete(critique) => {
            let payload = serde_json::to_string(&mail::response_email(&assignment,incoming,&problem.title,&critique))?;
        ...
        Progress::Failed(_) => {
            ...
            let payload = serde_json::to_string(&mail::response_email(&assignment,incoming,&problem.title,body))?;
            store.queue_response(incoming,&payload,crate::now())?;
            remove_scratch(root,&job_id);
...
async fn send_pending(store: &Store, email: &Client) -> Result<()> {
    ...
    for outgoing in store.outgoing(crate::now())? {
        let now = crate::now();
        if outgoing.expires_at<=now || outgoing.first_attempt.is_some_and(|first|first+SEND_WINDOW<=now) { continue; }
        let frozen: FrozenEmail = serde_json::from_str(&outgoing.payload).map_err(|_|fail("pending mail payload is invalid"))?;
        store.begin_send(&outgoing.id,now)?;
        match email.send_with_options(&frozen.message,&[],&frozen.reply).await {
            Ok(receipt) => store.sent(&outgoing,&receipt.id,crate::now())?,
            Err(_) => {
    ...
    if unresolved { Err(fail("email submission is unresolved")) } else { Ok(()) }
}
