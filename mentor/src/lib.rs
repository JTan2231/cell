...
/// Read the current user's absolute home path from the environment.
///
/// # Errors
/// Rejects an absent or relative `HOME` value.
...
/// Locate the mail service's state without reading desktop Mentor data.
///
/// # Errors
/// Rejects an absent or relative `HOME` value.
...
/// Encode 16 OS-random bytes as a lowercase hexadecimal routing token.
///
/// # Errors
/// Returns an error if the OS random device cannot supply all 16 bytes.
