...
impl Config {
    ...
    pub fn due_date(&self, now: i64) -> Result<Option<String>> {
        self.validate()?;
        let utc = chrono::DateTime::from_timestamp(now, 0).ok_or_else(|| fail("invalid current time"))?;
        let local = utc.with_timezone(&self.timezone.parse::<chrono_tz::Tz>()?);
        if (local.hour(), local.minute()) < (self.hour, self.minute) {
            return Ok(None);
        }
        Ok(Some(format!("{:04}-{:02}-{:02}", local.year(), local.month(), local.day())))
    }
}
...
pub fn private_directory(path: &Path) -> Result<()> {
    ...
    match fs::symlink_metadata(path) {
        Ok(meta) if meta.is_dir() && !meta.file_type().is_symlink() && meta.mode() & 0o077 == 0 => Ok(()),
        Ok(_) => Err(fail("state directory must be private and must not be a symlink")),
...
fn private_file(path: &Path, create: bool) -> Result<File> {
    ...
    if !meta.is_file() || meta.file_type().is_symlink() || meta.nlink() != 1 || meta.mode() & 0o077 != 0 {
...
pub fn runner_lock(root: &Path) -> Result<File> {
    ...
    file.try_lock_exclusive().map_err(|_| fail("another Mentor operation is active"))?;
    Ok(file)
}
...
impl Store {
    pub fn initialize(root: &Path) -> Result<Self> {
        ...
        let version: i64 = store.connection.pragma_query_value(None, "user_version", |row| row.get(0))?;
        if version == 0 {
            ...
            if tables != 0 {
                return Err(fail("unversioned nonempty database is not a Mentor database"));
            }
            ...
            transaction.execute("INSERT INTO corpora(id,body) VALUES(?1,?2)", params![corpus_id,serde_json::to_string(&corpus)?])?;
            for (key,value) in [("config",config),("shuffle_seed",seed),("active_corpus",corpus_id)] {
                transaction.execute("INSERT INTO metadata(key,value) VALUES(?1,?2)", params![key,value])?;
            }
            transaction.commit()?;
        } else if version != 1 {
    ...
    pub fn open(root: &Path) -> Result<Self> {
        ...
        let store = Self { connection: Connection::open_with_flags(path, OpenFlags::SQLITE_OPEN_READ_WRITE)? };
        store.configure_connection()?;
        let version: i64 = store.connection.pragma_query_value(None, "user_version", |row| row.get(0))?;
        if version != 1 { return Err(fail("initialize a supported Mentor database first")); }
        Ok(store)
    }
    ...
    pub fn meta(&self, key: &str) -> Result<Option<String>> {
        Ok(self.connection.query_row("SELECT value FROM metadata WHERE key=?1", [key], |row| row.get(0)).optional()?)
    }
    ...
    pub fn config(&self) -> Result<Config> {
        let config: Config = serde_json::from_str(&self.meta("config")?.ok_or_else(|| fail("Mentor configuration is absent"))?)?;
        config.validate()?;
        Ok(config)
    }
    ...
    pub fn import_corpus(&self, corpus: &Corpus) -> Result<String> {
        ...
        transaction.execute("INSERT OR IGNORE INTO corpora(id,body) VALUES(?1,?2)", params![id, serde_json::to_string(corpus)?])?;
    ...
    pub fn corpus(&self, id: &str) -> Result<Corpus> {
        let body: String = self.connection.query_row("SELECT body FROM corpora WHERE id=?1", [id], |row| row.get(0))?;
        Corpus::from_json(&body)
    }

    pub fn active_corpus(&self) -> Result<(String, Corpus)> {
        let id = self.meta("active_corpus")?.ok_or_else(|| fail("no active problem collection"))?;
        let corpus = self.corpus(&id)?;
        Ok((id, corpus))
    }

    pub fn assignment(&self, token: &str) -> Result<Option<Assignment>> {
        Ok(self.connection.query_row("SELECT date,problem_id,corpus_id,token,reply_to FROM assignments WHERE token=?1", [token], assignment_row).optional()?)
    }

    pub fn assignment_date(&self, date: &str) -> Result<Assignment> {
        Ok(self.connection.query_row("SELECT date,problem_id,corpus_id,token,reply_to FROM assignments WHERE date=?1", [date], assignment_row)?)
    }

    pub fn assigned_today(&self, date: &str) -> Result<bool> {
        Ok(self.connection.query_row("SELECT EXISTS(SELECT 1 FROM assignments WHERE date=?1)", [date], |row| row.get(0))?)
    }

    pub fn used_problem_ids(&self) -> Result<std::collections::HashSet<String>> {
        let mut statement = self.connection.prepare("SELECT problem_id FROM assignments")?;
        Ok(statement.query_map([], |row| row.get(0))?.collect::<rusqlite::Result<_>>()?)
    }

    pub fn reserve_assignment(&self, assignment: &Assignment, payload: &str, now: i64) -> Result<()> {
        let transaction = self.connection.unchecked_transaction()?;
    ...
    pub fn seen(&self, id: &str) -> Result<bool> {
        Ok(self.connection.query_row("SELECT EXISTS(SELECT 1 FROM incoming WHERE id=?1)", [id], |row| row.get(0))?)
    }
    ...
    pub fn capture(&self, incoming: &Incoming, now: i64, response: Option<&str>) -> Result<()> {
        ...
        if let Some(payload) = response.filter(|_|inserted==1) {
            ...
            transaction.execute("UPDATE incoming SET state='responding',answer=NULL WHERE id=?1", [&incoming.id])?;
        }
        transaction.commit()?;
        Ok(())
    }
    ...
    pub fn pending(&self) -> Result<Vec<Incoming>> {
        ...
        Ok(statement.query_map([], incoming_row)?.collect::<rusqlite::Result<_>>()?)
    }
    ...
    pub fn queue_response(&self, incoming: &Incoming, payload: &str, now: i64) -> Result<()> {
        ...
        transaction.execute("UPDATE incoming SET state='responding',answer=NULL,request_json=NULL WHERE id=?1", [&incoming.id])?;
        transaction.commit()?;
        Ok(())
    }
    ...
    pub fn cancellation_jobs(&self) -> Result<Vec<(String,String)>> {
        let mut statement = self.connection.prepare("SELECT id,job_id FROM incoming WHERE cancel_pending=1 AND job_id IS NOT NULL LIMIT 20")?;
        Ok(statement.query_map([], |row| Ok((row.get(0)?,row.get(1)?)))?.collect::<rusqlite::Result<_>>()?)
    }

    pub fn finish_cancellation(&self, id: &str) -> Result<()> {
        self.connection.execute("UPDATE incoming SET cancel_pending=0 WHERE id=?1", [id])?;
        Ok(())
    }
    ...
    pub fn outgoing(&self, now: i64) -> Result<Vec<Outgoing>> {
        ...
        Ok(statement.query_map([now], |row| Ok(Outgoing { id:row.get(0)?, incoming_id:row.get(1)?, payload:row.get(2)?, first_attempt:row.get(3)?, attempts:row.get(4)?, expires_at:row.get(5)? }))?.collect::<rusqlite::Result<_>>()?)
    }
    ...
    pub fn status(&self) -> Result<Value> {
        ...
        let remaining = corpus.problems.iter().filter(|p| !used.contains(&p.id)).count();
        let counts = |sql: &str| -> Result<Value> {
            let mut statement = self.connection.prepare(sql)?;
            let rows = statement.query_map([], |row| Ok((row.get::<_,String>(0)?,row.get::<_,i64>(1)?)))?;
            let mut values = serde_json::Map::new();
            for row in rows { let (state,count) = row?; values.insert(state,json!(count)); }
            Ok(Value::Object(values))
        };
        Ok(json!({"paused":config.paused,"daily_time":format!("{:02}:{:02}",config.hour,config.minute),"timezone":config.timezone,
...
fn assignment_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<Assignment> {
    Ok(Assignment { date:row.get(0)?,problem_id:row.get(1)?,corpus_id:row.get(2)?,token:row.get(3)?,reply_to:row.get(4)? })
}

fn incoming_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<Incoming> {
    let refs: String = row.get(6)?;
    let references = serde_json::from_str(&refs).map_err(|e|rusqlite::Error::FromSqlConversionFailure(6,rusqlite::types::Type::Text,Box::new(e)))?;
    Ok(Incoming { id:row.get(0)?,assignment_date:row.get(1)?,state:row.get(2)?,received_at:row.get(3)?,expires_at:row.get(4)?,message_id:row.get(5)?,references,answer:row.get(7)?,request_json:row.get(8)?,job_id:row.get(9)? })
}
