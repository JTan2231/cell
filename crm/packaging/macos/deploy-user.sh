...
if [ -d "$SCRIPT_DIR/../share/chancery/crm" ]; then
    SOURCE_CHANCERY="$SCRIPT_DIR/../share/chancery/crm"
else
...
Install or update the current user's macOS CRM CLI and product-owned Chancery
provider. Deployment does not initialize or migrate CRM storage or launch its
hidden worker.
...
operator_uid=$(id -u)
[ "$operator_uid" -ne 0 ] || fail 'run as the CRM operator, not root'
...
validate_bundle() {
    ...
    expected_bundle_tree=$(printf '%s\n' \
        . \
        ./entries \
        ./entries/case-maintain.json \
        ./entries/develop-change.json \
        ./entries/library-explore.json \
        ./entries/steward-operate.json \
        ./manuals \
        ./manuals/case-maintain.md \
        ./manuals/develop-change.md \
        ./manuals/library-explore.md \
        ./manuals/steward-operate.md \
        ./provider.json \
        | LC_ALL=C sort)
    [ "$actual_bundle_tree" = "$expected_bundle_tree" ] \
        || fail "Chancery bundle tree is not the exact CRM v0.1 layout: $bundle"
...
validate_release_selector() {
    ...
    printf '%s\n' "$selector" | grep -Eq '^releases/[0-9a-f]{64}$' \
        || fail "invalid CRM release selector: $selector"
}
...
validate_installed_release() {
    ...
    for directory in "$inspected_path" "$inspected_path/bin" \
        ...
        "$inspected_path/share/chancery/crm"; do
    ...
    expected_release_tree=$(printf '%s\n' \
        ...
        ./share/chancery \
        ./share/chancery/crm \
        ./share/chancery/crm/entries \
        ./share/chancery/crm/entries/case-maintain.json \
        ./share/chancery/crm/entries/develop-change.json \
        ./share/chancery/crm/entries/library-explore.json \
        ./share/chancery/crm/entries/steward-operate.json \
        ./share/chancery/crm/manuals \
        ./share/chancery/crm/manuals/case-maintain.md \
        ./share/chancery/crm/manuals/develop-change.md \
        ./share/chancery/crm/manuals/library-explore.md \
        ./share/chancery/crm/manuals/steward-operate.md \
        ./share/chancery/crm/provider.json \
        | LC_ALL=C sort)
    [ "$actual_release_tree" = "$expected_release_tree" ] \
        || fail "installed release tree is not the exact CRM v0.1 layout: $selector"
    [ -f "$inspected_path/bin/crm" ] \
        && [ ! -L "$inspected_path/bin/crm" ] \
        && [ -x "$inspected_path/bin/crm" ] \
    ...
    validate_bundle "$inspected_path/share/chancery/crm"
    ...
    [ "$(sed -n '2p' "$inspected_manifest")" = 'product=crm' ] \
...
candidate_version=$("$binary_path" --version) \
    || fail 'unable to read the CRM candidate version'
case "$candidate_version" in
    'crm '*) version=${candidate_version#crm } ;;
...
STATE_DIR="$install_home/Library/Application Support/CRM"
...
CLI_PATH="$CLI_DIR/crm"
CHANCERY_STATE="$install_home/Library/Application Support/Chancery"
CHANCERY_PROVIDERS="$CHANCERY_STATE/providers"
CHANCERY_LINK="$CHANCERY_PROVIDERS/crm"
EXPECTED_CLI="$INSTALL_DIR/current/bin/crm"
EXPECTED_CHANCERY="$INSTALL_DIR/current/share/chancery/crm"
...
cleanup() {
    ...
    set +e
    if [ "$exit_status" -ne 0 ] && [ "$committed" -eq 0 ] \
        && [ "$switched" -eq 1 ]; then
...
mkdir "$UPDATE_LOCK" 2>/dev/null \
    || fail "another CRM deployment is active: $UPDATE_LOCK"
lock_created=1
...
if [ -n "$old_current" ]; then
    ...
    fail 'installed selectors have no current CRM release'
fi
...
installed_version=$(HOME="$install_home" "$CLI_PATH" --version) \
    || fail 'installed CRM version check failed'
[ "$installed_version" = "crm $version" ] \
    || fail "installed CRM reported an unexpected version: $installed_version"
HOME="$install_home" "$CLI_PATH" --help >/dev/null \
    || fail 'installed CRM help check failed'
[ -f "$CHANCERY_LINK/provider.json" ] \
    || fail 'installed CRM Chancery provider is unavailable'
validate_installed_release "$(readlink "$CURRENT_LINK")"

committed=1
printf 'Installed CRM release %s\n' "$release_id"
printf 'Command: %s\n' "$CLI_PATH"
printf 'Chancery provider: %s\n' "$CHANCERY_LINK"
printf '%s\n' 'Database: not initialized by deployment; run `crm init` separately'
