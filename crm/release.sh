...
manifest_path=crm/crates/crm/Cargo.toml
provider_path=crm/chancery/provider.json
ci_path=crm/ci.sh
...
reported_version=$(target/release/crm --version) \
    || fail 'unable to read release binary version'
[ "$reported_version" = "crm $new_version" ] \
...
if ! git tag -a "$tag" -m "CRM v$new_version"; then
