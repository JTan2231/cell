...
manifest_path=conversations/crates/conversations/Cargo.toml
provider_path=conversations/chancery/provider.json
ci_path=conversations/ci.sh
...
tag="conversations-v$new_version"
...
reported_version=$(target/release/conversations --version) \
    || fail 'unable to read release binary version'
[ "$reported_version" = "conversations $new_version" ] \
...
if ! git tag -a "$tag" -m "Conversations v$new_version"; then
...
printf 'Released Conversations %s\n' "$new_version"
