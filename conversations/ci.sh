...
if [ "${CONVERSATIONS_CI_TIMEOUT_ACTIVE:-0}" != 1 ]; then
    export CONVERSATIONS_CI_TIMEOUT_ACTIVE=1
    timeout_command=
    ...
    printf 'ci.sh: timeout or Perl is required to enforce %s seconds\n' \
        "$MAX_SECONDS" >&2
    exit 1
fi
...
[ -f "$WORKSPACE_MANIFEST" ] || { printf '%s\n' 'ci.sh: root Cargo.toml is required' >&2; exit 1; }
[ -f "$WORKSPACE_LOCK" ] || { printf '%s\n' 'ci.sh: root Cargo.lock is required' >&2; exit 1; }
export CARGO_BUILD_WARNINGS=deny

printf '%s\n' '==> shell and packaging'
for script in release.sh packaging/macos/deploy-user.sh packaging/macos/test-deploy-user.sh; do
    [ -f "$script" ] || { printf 'ci.sh: missing %s\n' "$script" >&2; exit 1; }
    sh -n "$script"
done
...
[ -n "$package_version" ] && [ "$provider_release" = "$package_version" ] || {
    printf 'ci.sh: provider %s does not match package %s\n' \
...
printf '%s\n' '==> rustfmt'
cargo fmt --manifest-path "$WORKSPACE_MANIFEST" --package conversations -- --check

printf '%s\n' '==> clippy'
cargo clippy --manifest-path "$WORKSPACE_MANIFEST" \
    --package conversations --all-targets --locked --keep-going -- \
    -D warnings \
    -F unsafe_code \
    -D clippy::all \
    -D clippy::pedantic \
...
cargo test --manifest-path "$WORKSPACE_MANIFEST" \
    --package conversations --locked --no-fail-fast

printf '%s\n' '==> rustdoc'
RUSTDOCFLAGS='-D warnings' cargo doc --manifest-path "$WORKSPACE_MANIFEST" \
    --package conversations --no-deps --locked

printf '%s\n' '==> release build'
cargo build --manifest-path "$WORKSPACE_MANIFEST" \
    --package conversations --release --locked
