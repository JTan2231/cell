...
fn installer_preflight(
    ...
    if operation == "recover" {
        return Err(fail(
            "direct selector recovery is unsupported for EMT state; use coordinated deployment recovery",
        ));
    }
    ...
    if root.join("emt.sqlite3").try_exists()? {
        return Err(fail(
            "initialized EMT installations must be updated through coordinated deployment",
        ));
    }
