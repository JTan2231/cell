...
SOURCE_FRONTEND="$SCRIPT_DIR/cast"
if [ -d "$SCRIPT_DIR/../share/chancery/cast" ]; then
    SOURCE_CHANCERY="$SCRIPT_DIR/../share/chancery/cast"
else
...
Install or update the current user's macOS Cast CLI. Deployment changes only program and documentation selectors. It does not
initialize discovery storage, collect sources, or activate a schedule.
...
[ -f "$binary_path" ] && [ ! -L "$binary_path" ] && [ -x "$binary_path" ] \
    || fail "Cast candidate is not an executable regular file: $binary_path"
...
operator_uid=$(id -u)
[ "$operator_uid" -ne 0 ] || fail 'run this deployer as the Cast operator, not root'
...
candidate_version=$("$binary_path" --version) \
    || fail 'unable to read the Cast candidate version'
case "$candidate_version" in
    'cast '*) version=${candidate_version#cast } ;;
    *) fail "Cast candidate reported an unexpected version: $candidate_version" ;;
esac
[ -n "$version" ] || fail 'Cast candidate reported an empty version'
...
"$binary_path" --help >/dev/null \
    || fail 'unable to read the Cast candidate help'
sh -n "$SOURCE_DEPLOYER"
/bin/zsh -n "$SOURCE_FRONTEND"

STATE_DIR="$install_home/Library/Application Support/Cast"
...
CLI_PATH="$CLI_DIR/cast"
CHANCERY_STATE_DIR="$install_home/Library/Application Support/Chancery"
CHANCERY_PROVIDERS_DIR="$CHANCERY_STATE_DIR/providers"
CHANCERY_PROVIDER_LINK="$CHANCERY_PROVIDERS_DIR/cast"
CHANCERY_PROVIDER_TARGET="$INSTALL_DIR/current/share/chancery/cast"
...
mkdir "$UPDATE_LOCK" 2>/dev/null \
    || fail "another Cast deployment is active: $UPDATE_LOCK"
lock_created=1
...
if [ -n "$old_current" ]; then
    ...
    [ "$old_cli" = "$INSTALL_DIR/current/bin/cast" ] \
        || fail "installed command does not select the current release: $CLI_PATH"
elif [ -n "$old_cli" ] || [ -n "$old_previous" ]; then
    fail 'installed selectors have no current Cast release'
fi
if [ -n "$old_provider" ] && [ "$old_provider" != "$CHANCERY_PROVIDER_TARGET" ]; then
    fail "provider selector is not owned by this Cast installation: $CHANCERY_PROVIDER_LINK"
fi
...
validate_selected_release() {
    selected=$1
    [ -n "$selected" ] || return 0
    case "$selected" in releases/*) ;; *) fail "invalid release selector: $selected" ;; esac
    selected_id=${selected#releases/}
    [ "${#selected_id}" -eq 64 ] || fail "invalid release selector: $selected"
    case "$selected_id" in *[!0-9a-f]*) fail "invalid release selector: $selected" ;; esac
    selected_path="$INSTALL_DIR/$selected"
    [ -d "$selected_path" ] && [ ! -L "$selected_path" ] \
        || fail "selected release is not an owned directory: $selected"
    if find "$selected_path" ! -type d ! -type f -print | grep -q .; then
        fail "selected release contains a non-regular object: $selected"
    fi
    for relative in bin/cast libexec/cast package/cast package/deploy-user.sh; do
        [ -f "$selected_path/$relative" ] && [ -x "$selected_path/$relative" ] \
            || fail "selected release lacks executable $relative"
    done
    [ -f "$selected_path/manifest.txt" ] \
        || fail 'selected release has no manifest'
    selected_payload=$(shasum -a 256 "$selected_path/libexec/cast" | awk '{print $1}')
    selected_frontend=$(shasum -a 256 "$selected_path/bin/cast" | awk '{print $1}')
    selected_deployer=$(shasum -a 256 "$selected_path/package/deploy-user.sh" | awk '{print $1}')
    selected_provider=$(hash_tree "$selected_path/share/chancery/cast")
    [ "$(shasum -a 256 "$selected_path/package/cast" | awk '{print $1}')" = "$selected_frontend" ] \
        || fail 'selected release packaged frontend differs'
    calculated_id=$(printf '%s\n' "$selected_payload" "$selected_frontend" \
        "$selected_deployer" "$selected_provider" | shasum -a 256 | awk '{print $1}')
    [ "$calculated_id" = "$selected_id" ] \
        || fail "selected release integrity failed: $selected"
    selected_version=$(awk -F '=' '$1 == "version" {print $2}' "$selected_path/manifest.txt")
    expected_manifest=$(printf '%s\n' 'format=1' "release_id=$selected_id" \
        "version=$selected_version" "payload_sha256=$selected_payload" \
        "frontend_sha256=$selected_frontend" "deployer_sha256=$selected_deployer" \
        "provider_sha256=$selected_provider")
    [ "$(cat "$selected_path/manifest.txt")" = "$expected_manifest" ] \
        || fail 'selected release manifest is invalid'
}

validate_selected_release "$old_current"
validate_selected_release "$old_previous"
...
if [ -d "$release_path" ]; then
    [ -x "$release_path/bin/cast" ] && [ ! -L "$release_path/bin/cast" ] \
        || fail "existing release contains no valid Cast wrapper: $release_id"
    [ -x "$release_path/libexec/cast" ] && [ ! -L "$release_path/libexec/cast" ] \
        || fail "existing release contains no valid Cast payload: $release_id"
    [ -x "$release_path/package/cast" ] && [ ! -L "$release_path/package/cast" ] \
    ...
    [ "$(shasum -a 256 "$release_path/libexec/cast" | awk '{print $1}')" = "$payload_hash" ] \
        || fail "existing release payload hash is invalid: $release_id"
    [ "$(shasum -a 256 "$release_path/bin/cast" | awk '{print $1}')" = "$frontend_hash" ] \
        || fail "existing release wrapper hash is invalid: $release_id"
    [ "$(shasum -a 256 "$release_path/package/cast" | awk '{print $1}')" = "$frontend_hash" ] \
    ...
    [ -d "$release_path/share/chancery/cast" ] \
        && [ ! -L "$release_path/share/chancery/cast" ] \
        || fail "existing release contains no valid provider bundle: $release_id"
    [ "$(hash_tree "$release_path/share/chancery/cast")" = "$provider_hash" ] \
    ...
    install -m 0755 "$SOURCE_FRONTEND" "$temporary_release/bin/cast"
    install -m 0755 "$binary_path" "$temporary_release/libexec/cast"
    install -m 0755 "$SOURCE_DEPLOYER" "$temporary_release/package/deploy-user.sh"
    install -m 0755 "$SOURCE_FRONTEND" "$temporary_release/package/cast"
    cp -R "$SOURCE_CHANCERY" "$temporary_release/share/chancery/cast"
    find "$temporary_release/share/chancery/cast" -type d -exec chmod 0755 {} \;
    find "$temporary_release/share/chancery/cast" -type f -exec chmod 0444 {} \;
...
atomic_symlink "$INSTALL_DIR/current/bin/cast" "$CLI_PATH"
atomic_symlink "$CHANCERY_PROVIDER_TARGET" "$CHANCERY_PROVIDER_LINK"

HOME="$install_home" "$CLI_PATH" --version >/dev/null \
    || fail 'installed Cast version check failed'
HOME="$install_home" "$CLI_PATH" --help >/dev/null \
    || fail 'installed Cast help check failed'

committed=1
printf 'Installed Cast release %s\n' "$release_id"
printf 'Command:  %s\n' "$CLI_PATH"
